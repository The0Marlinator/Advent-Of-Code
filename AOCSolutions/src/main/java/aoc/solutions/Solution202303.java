...
public class Solution202303 extends AOCSolution {

    public Solution202303(boolean printOutput) {
        super(printOutput);
    }
    ...
    private List<List<Integer>> borderingPlacesHaveNumbers(List<List<List<Integer>>> starLocations) {
        ...
        for (int star_y = 0; star_y < starLocations.size(); ++star_y) {
            for (int star_x = 0; star_x < starLocations.get(star_y).size(); ++star_x) {
                ...
                if (star_y - 1 >= 0) {
                    for (List<Integer> numberIndicesOnRow : numberLocations.get(star_y - 1)) {
                        if (!adjacentNumbers.contains(buildNumberFromIntegerList(numberIndicesOnRow, star_y - 1)) && (numberIndicesOnRow.contains(starXActual - 1) || numberIndicesOnRow.contains(starXActual) || numberIndicesOnRow.contains(starXActual + 1))) {
                            adjacentNumbers.add(buildNumberFromIntegerList(numberIndicesOnRow, star_y - 1));
                        }
                    }
                }

                for (List<Integer> numberIndicesOnRow : numberLocations.get(star_y)) {
                    if (!adjacentNumbers.contains(buildNumberFromIntegerList(numberIndicesOnRow, star_y)) && (numberIndicesOnRow.contains(starXActual - 1) || numberIndicesOnRow.contains(starXActual) || numberIndicesOnRow.contains(starXActual + 1))) {
                        adjacentNumbers.add(buildNumberFromIntegerList(numberIndicesOnRow, star_y));
                    }
                }
