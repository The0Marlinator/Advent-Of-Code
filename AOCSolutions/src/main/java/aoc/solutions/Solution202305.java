...
@Solution(year = 2023, day = 5)
public class Solution202305 extends AOCSolution {
    ...
    public Solution202305(boolean printOutput) {
        super(printOutput);
    ...
    private long getSmallestLocationFromRange(Range range, String mapIndex) {
        ...
        for (Pair<Range, Range> map : mapList) {
            ...
            for (Range unmappedRange : unmapped) {
                Optional<Range> overLappingRange = map.first().getOverlapping(unmappedRange);
                ...
                printToOutput(() -> "Step: " + mapIndex + " input:" + unmappedRange + " overlapping:" + overLappingRange + " source map:" + map.first() + " destination map:" + map.second() + " mapped:" + mapRangeToRangePair(overLappingRange.get(), map) + " unmapped: " + newUnmapped);
                smallest = Math.min(smallest, getSmallestLocationFromRange(mapRangeToRangePair(overLappingRange.get(), map), getNextMapIndex(mapIndex)));
            }
            unmapped = newUnmapped;
        }
        List<Range> finalUnmapped = unmapped;
        printToOutput(() -> "Step:" + mapIndex + " Handling unmapped values that are left: " + finalUnmapped);
